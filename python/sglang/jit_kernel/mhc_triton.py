...
def mhc_pre_triton(
    ...
    """Drop-in Triton replacement for `srt/layers/mhc.py:mhc_pre`. Same
    dispatch tree (split-K vs simple based on num_tokens), same outputs.

    Returns (post_mix [..., hc_mult, 1], comb_mix [..., hc_mult, hc_mult],
    layer_input [..., hidden]).
    """
    ...
    # Stage 1: GEMM + sqrsum. Mirrors mhc.py:552-586 dispatch.
    x_flat = residual_flat.view(num_tokens, hc_hidden_size)
    if num_tokens <= 2048:
        assert n_splits == 1, "split-K path supports n_splits == 1 only"
        gemm_out_2d, gemm_sqr_1d = mhc_pre_gemm_sqrsum_splitk_triton(
            x_flat, fn, hc_mult3=hc_mult3, n_splits_pre=n_splits_pre,
        )
    else:
        assert n_splits == 1
        gemm_out_2d, gemm_sqr_1d = mhc_pre_gemm_sqrsum_triton(
            x_flat, fn, hc_mult3=hc_mult3,
        )
...
def mhc_post_triton_full(
    x: torch.Tensor,
    residual: torch.Tensor,
    post_layer_mix: torch.Tensor,
    comb_res_mix: torch.Tensor,
) -> torch.Tensor:
    """Drop-in replacement for `srt/layers/mhc.py:mhc_post`. Same I/O
    semantics; thin wrapper around `mhc_post_triton` that handles the
    `[n, hc, 1]`/`[n, hc]` post_layer_mix variants used by the live
    caller (see models/deepseek_v4.py:1813 squeeze).
    """
